...
# Inicialização estável de bases CSV caso não existam
...
def obter_imagem_base64_flexivel():
    ...
    nomes_fallback = ["Foto perfil Matheus.jpg", "foto perfil matheus.jpg", "Foto_Perfil.jpg"]
    ...
    if not df_vagas.empty:
        ...
        for idx, row in df_vagas.iterrows():
            with colunas_vagas[idx % len(df_vagas)]:
                st.markdown(f'<div class="focus-card"><h3 style="margin-top:0;">{row["Título"]}</h3><p style="font-size:0.95rem; color:#94A3B8; line-height:1.5;">{row["Descrição"]}</p></div>', unsafe_allow_html=True)
...
with aba_conhecimentos:
    ...
    with col_k1:
        st.markdown('<div class="skill-section-card"><h3>📊 BI, Engenharia & Ciência de Dados</h3>', unsafe_allow_html=True)
        for _, row in df_skills[df_skills["Categoria"] == "Dados"].iterrows():
        ...
        st.markdown('<div class="skill-section-card"><h3>🏢 Engenharia de Automação (RPA)</h3>', unsafe_allow_html=True)
        for _, row in df_skills[df_skills["Categoria"] == "RPA"].iterrows():
            st.markdown(f'<div class="skill-item"><div class="skill-name-percentage"><span>{row["Nome"]}</span><span>{row["Porcentagem"]}%</span></div><div class="skill-bar-bg"><div class="skill-bar-fill" style="width: {row["Porcentagem"]}%;"></div></div></div>', unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)

    with col_k2:
        st.markdown('<div class="skill-section-card"><h3>🏢 Ecossistema ERP SAP & Corporativo</h3>', unsafe_allow_html=True)
        for _, row in df_skills[df_skills["Categoria"] == "SAP"].iterrows():
            st.markdown(f'<div class="skill-item"><div class="skill-name-percentage"><span>{row["Nome"]}</span><span>{row["Porcentagem"]}%</span></div><div class="skill-bar-bg"><div class="skill-bar-fill" style="width: {row["Porcentagem"]}%;"></div></div></div>', unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
...
with aba_projetos:
    ...
    if not df_dados.empty:
        for index, row in df_dados.iterrows():
            st.markdown(f'<div class="project-card"><h4 style="margin:0 0 10px 0; color:#38BDF8 !important;">🚀 {row["Título"]} <small style="color:#64748B; font-size:0.8rem;">({row["Categoria"]})</small></h4><p style="color:#94A3B8; font-size:0.95rem; margin-bottom:15px;">{row["Descrição"]}</p></div>', unsafe_allow_html=True)
...
with aba_formacao:
    ...
    st.markdown('<div style="background:#1E293B; padding:24px; border-radius:12px; border-left:5px solid #38BDF8; margin-bottom: 25px;"><h4 style="margin:0; color:white; font-size:1.2rem;">Bacharelado em Tecnologia da Informação</h4><span style="color:#38BDF8; font-weight:500;">Universidade Virtual do Estado de São Paulo (UNIVESP)</span><p style="margin:8px 0 0 0; color:#94A3B8;">Status: <strong>Ensino Superior Completo / Graduado</strong></p></div>', unsafe_allow_html=True)
    ...
    cursos = [
        ...
        ("🏢 Fundamentos Básicos do SAP S/4HANA", "KA Solutions", "Carga Horária: 8 hours", "Arquitetura estrutural base de sistemas integrados ERP.")
    ]
    for titulo, emissor, data, desc in cursos:
        st.markdown(f'<div class="course-card"><strong style="color:#F8FAFC; font-size:1.05rem;">{titulo}</strong><br><span style="color:#38BDF8; font-size:0.9rem;">{emissor} — <small style="color:#64748B;">{data}</small></span><p style="margin:6px 0 0 0; color:#94A3B8; font-size:0.9rem;">{desc}</p></div>', unsafe_allow_html=True)
...
if st.session_state["autenticado"]:
    ...
    menu_adm = st.selectbox("Escolha a Base para Modificar", ["Projetos e Automações", "Focos de Vagas (Objective)", "Novos Conhecimentos Técnicos", "🖼️ Atualizar Foto de Perfil"])
    ...
    if menu_adm == "Projetos e Automações":
        ...
        if not df_dados.empty:
            ...
            confirmar_exclusao = st.checkbox("⚠️ Confirmar exclusão do registro de dados", key="conf_ex_dados")
            if st.button("❌ Apagar Registro"):
                if confirmar_exclusao:
                    ...
                    st.warning("Marque a caixa de verificação para prosseguir.")
        else:
            st.info("Nenhum projeto cadastrado.")

    # 2. GERENCIAR VAGAS
    elif menu_adm == "Focos de Vagas (Objective)":
        ...
        if not df_vagas.empty:
            ...
            confirmar_exclusao = st.checkbox("⚠️ Confirmar exclusão deste foco estrutural", key="conf_ex_vagas")
    ...
    # 3. GERENCIAR CONHECIMENTOS
        ...
        if not df_skills.empty:
            ...
            confirmar_exclusao = st.checkbox("⚠️ Confirmar exclusão desta skill da matriz", key="conf_ex_skills")
    ...
    # 4. ATUALIZAR FOTO DE PERFIL
        ...
        st.write("Selecione sua foto profissional. Ela será processada com isolamento de escopo.")
        ...
        if foto_carregada is not None:
            ...
            if st.button("💾 Aplicar Nova Imagem ao Perfil", type="primary"):
                try:
                    ...
                    st.error(f"Erro ao salvar arquivo de imagem na nuvem: {e}")    for nome in nomes_fallback:
        if os.path.exists(nome):
            try:
                with open(nome, "rb") as image_file:
                    return base64.b64encode(image_file.read()).decode()
...
# --- FUNÇÃO AUXILIAR DE RENDERIZAÇÃO DA IMAGEM (Removido Glob perigoso da Nuvem) ---
def obter_imagem_base64_flexivel():
    # Verifica primeiro se há um upload ativo salvo localmente de forma explícita
    if os.path.exists(NOME_ESTÁTICO_FOTO):
        try:
            with open(NOME_ESTÁTICO_FOTO, "rb") as image_file:
                return base64.b64encode(image_file.read()).decode()
    ...
    # Fallback para os nomes padrões que você possa ter enviado inicialmente via Git
    nomes_fallback = ["Foto perfil Matheus.jpg", "foto perfil matheus.jpg", "Foto_Perfil.jpg"]
    for nome in nomes_fallback:
        if os.path.exists(nome):
            try:
...
if st.session_state["autenticado"]:
    ...
    menu_adm = st.selectbox("Escolha a Base para Modificar", ["Projetos e Automações", "Focos de Vagas (Objetivo)", "Novos Conhecimentos Técnicos", "🖼️ Atualizar Foto de Perfil"])
    ...
    # 4. ATUALIZAR FOTO DE PERFIL (Seguro contra travas da nuvem)
    elif menu_adm == "🖼️ Atualizar Foto de Perfil":
        st.subheader("Substituir Imagem do Perfil")
        st.write("Selecione sua foto profissional. Ela será processada com isolamento de escopo.")
        ...
        if foto_carregada is not None:
            ...
            if st.button("💾 Aplicar Nova Imagem ao Perfil", type="primary"):
                try:
                    # Sobrescreve apenas o arquivo estático isolado, sem o uso do glob recursivo que quebra as permissões do SO
                    if os.path.exists(NOME_ESTÁTICO_FOTO):
                        os.remove(NOME_ESTÁTICO_FOTO)
                    ...
                    st.error(f"Erro ao salvar arquivo de imagem na nuvem: {e}")    ])
    df_vagas_init.to_csv(ARQUIVO_VAGAS, index=False)
